...
def calculateFEM(numElementNodes, H, L, thickness, eq, endLoadXY, ep, Dmat, numNodesX, numNodesY, prints=False, bDrawMesh=False):
    """Calculates a centilever beam with the given options. Can draw mesh and displaced mesh.
    Returns data about reaction forces, nodes and displacements in an array

    Args:
        numElementNodes (int): [3,33,4,44,6,9] noder per element. 33,44 are existing implementations.
        H (float): Height of beam
        L (float): length of beam
        thickness (float): Thickness of the beam
        eq ([float,float]): Distributed load in x and y, load pr unit area
        endLoadXY ([float,float]): a load applied to the right end of the cantilever
        ep ([int, float, int]): [analysis type, thickness, number of gausspoints] properties for calling functions
        Dmat (matrix): [description]
        numNodesX (int): [description]
        numNodesY (int): [description]
        prints (bool, optional): Print values to terminal. Defaults to False.
        bDrawMesh (bool, optional): Show meshes in new windows. Defaults to False.

    Returns:
        numpy array: [numElementNodes, numNodes, xC, yC, R0Sum[0], R0Sum[1], existingImpl(1./0.)]
    """
...
def append_row_toDict(dict, datarow):
    """Appends datarow to the dictionary

    Args:
        dict : works only with this one spesific dict.
        datarow ([floatx7]): 7 floats of data

    Returns:
        dict: returns updated dict
    """

    dict["Nods pr el"].append(datarow[0])
    dict["Tot nods"].append(datarow[1])
    dict["xC"].append(datarow[2])
    dict["yC"].append(datarow[3])
    dict["Tot.react.force x"].append(datarow[4])
    dict["Tot.react.force y"].append(datarow[5])
    dict["exist impl."].append(datarow[6])
...
#___________________________________________________________________________________________________________________
    ...
    for elt in elementTypes: # Calculate for all elementtypes
        ...
        for scaleNumNodes in range(40): # How many iterations with increasing number of nodes
            ...
            numNodesY = 4 + 2 * scaleNumNodes

            # Calculate end disp, tot. react. forces using cantilever calc
            dataRow = calculateFEM(elt, H, L, thickness, eq, endLoadXY, ep, Dmat, numNodesX, numNodesY)

            # Append data to dictionary
            append_row_toDict(dict, dataRow)

            print("Appended row", elt, scaleNumNodes)
